/*segment tree data structure with lazy propagation for finding max in an interval. 
  Lazy propagation is used when an entire subarray needs to be updated, otherwise the usual segment tree implementation will do fine*/
...
void update_segment_tree(int node, int b, int e, int i, int j,int val)
{
	if(b==e && b==x)
	{
		tree[node]+=val;
