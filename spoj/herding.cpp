...
public:
	...
	int printDFS(); //prints tree with origin u using the DFS algorithm
...
int main()
{
	...
	for(i=1;i<=n;i++)
	{
		...
		for(j=1;j<=m;j++)
		{
			...
			if(ch=='S')
				g.addEdge(MAP(i,j), k=MAP(i+1,j), false);
			else if(ch=='N')
				g.addEdge(MAP(i,j), k=MAP(i-1,j), false);
			else if(ch=='W')
				g.addEdge(MAP(i,j), k=MAP(i,j-1),false);
			else
				g.addEdge(MAP(i,j), k=MAP(i,j+1),false);
			debug("hl",i,j,MAP(i,j), k, ch);
		}		
	}
	cout<<g.printDFS()<<"\n";
	/*Graph g(8);
